...
keyTakeaways:
  ...
  - "Grounded Retrieval combines structured NCCN guideline navigation with real-time internet search, using the LLM for semantic extraction — not fact recall"
...
NCCN Clinical Practice Guidelines, the de facto standard of care in the United States, update on a rolling basis. The NCCN publishes over 80 guidelines covering virtually every cancer type, with major guidelines receiving 4–8 updates annually.<sup>3</sup> Each update can introduce new first-line therapies, remove options based on safety signals, or restructure entire treatment algorithms based on new trial data.
...
└──────────────────────────┬──────────────────────────────┘
              ...
              ▼                         ▼
┌──────────────────────┐   ┌──────────────────────┐
│  NCCN TOC Navigation │   │  Internet Search     │
...
The system uses a Thread Pool Executor to run NCCN guideline navigation and internet search in parallel. NCCN navigation isn't vector similarity — it's structured traversal through the table of contents, identifying the correct guideline, locating the relevant section, and extracting content with specific page numbers.
...
3. NCCN Clinical Practice Guidelines in Oncology. National Comprehensive Cancer Network. nccn.org.
