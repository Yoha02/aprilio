...
description: "NCCN guidelines aren't just documents — they're a clinical ontology. Here's why using them as a symbolic scaffold for LLM-based retrieval outperforms pure neural approaches."
keyTakeaways:
  - "NCCN guidelines provide a de facto cancer ontology: taxonomy, biomarker features, treatment algorithms, and evidence grading"
...
## NCCN Guidelines Are Not Just Documents

When most people think about clinical practice guidelines, they imagine long PDF documents that clinicians skim when time allows. But NCCN guidelines are something far more structured — they encode a clinical ontology that represents decades of expert consensus.

Consider what NCCN guidelines actually contain:
...
**Chunk boundaries destroy treatment algorithms.** NCCN treatment algorithms span multiple pages with complex branching logic. Arbitrary chunking at 512 or 1024 tokens fragments these decision trees into meaningless segments.
...
Grounded Retrieval takes a fundamentally different approach. Rather than treating NCCN guidelines as flat text for embedding, it uses their inherent structure as a navigation scaffold:

```
┌─────────────────────────────────────────────────────────┐
│              NCCN Ontology Layers                         │
├─────────────────────────────────────────────────────────┤
│                                                          │
...
**Clinical safety requires hard constraints.** A language model can generate any sequence of tokens — including recommendations for withdrawn drugs. Symbolic structures (like current NCCN guidelines) provide hard boundaries on what constitutes valid recommendations.

**Verification requires structured sources.** Telling a clinician "the AI says to use Drug X" is inadequate. Telling them "NCCN Guidelines, Version 2.2026, page 47 recommends Drug X as Category 1 evidence" enables direct verification. This level of citation is only possible with structured source navigation.
...
For oncology specifically, NCCN guidelines provide an ideal symbolic scaffold: they're authoritative, structured, regularly updated, and universally recognized as the standard of care. Using them as a navigation target rather than an embedding source transforms the LLM from an unreliable oracle into a precise retrieval interface.
