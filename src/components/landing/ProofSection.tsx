  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 font-mono text-[9px] uppercase tracking-wider text-white/40">{meta}</p>
      </div>
      <span className="font-mono text-[9px] text-cyan">linked</span>
    </div>
  )
}
  ...
  return (
    <section id="evidence" className="bg-paper py-24 sm:py-32">
      <Container>
          ...
          <div className="grid lg:grid-cols-[0.62fr_1.38fr]">
            <div className="border-b border-white/10 p-5 sm:p-7 lg:border-b-0 lg:border-r">
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="rounded-full bg-white/8 px-2.5 py-1 font-mono text-[9px] uppercase tracking-wider text-white/45">high stakes</span>
              </div>
              ...
              <div className="mt-8 border-t border-white/10 pt-5">
                <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-white/35">Why this query is hard</p>
              ...
              <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div className="inline-flex w-fit rounded-full border border-white/10 bg-white/[0.04] p-1" role="group" aria-label="Compare answer modes">
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-xs font-semibold transition-colors ${view === 'recall' ? 'bg-white text-ink' : 'text-white/50 hover:text-white'}`}
                  >
                    Model recall
                  </button>
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-xs font-semibold transition-colors ${view === 'trace' ? 'bg-cyan text-ink' : 'text-white/50 hover:text-white'}`}
                  >
                    Aprilio trace
                  </button>
                </div>
              ...
              <AnimatePresence mode="wait">
                {view === 'trace' ? (
                  <motion.div key="trace" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="mt-7 grid gap-4 xl:grid-cols-[1.2fr_.8fr]">
                    <div className="rounded-2xl border border-cyan/25 bg-white/[0.055] p-5 sm:p-6">
                      <div className="flex items-center gap-2">
                        <span className="rounded-md bg-factum px-2 py-1 font-mono text-[9px] font-bold uppercase tracking-wider text-ink">Factum</span>
                        <span className="font-mono text-[9px] uppercase tracking-wider text-white/35">current as retrieved</span>
                      </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="rounded-xl border border-success/20 bg-success/10 px-3 py-3">
                        <p className="font-mono text-[9px] font-bold uppercase tracking-wider text-success-light">Boundary preserved</p>
                        <p className="mt-1.5 text-xs leading-5 text-white/55">The system separates retrieved guidance from unreviewed external updates.</p>
                      </div>
                    </div>
                  </motion.div>
                ) : (
                  <motion.div key="recall" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="mt-7 rounded-2xl border border-coral/20 bg-coral/[0.06] p-5 sm:p-7">
                    <div className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-wider text-coral">
                    ...
                    <div className="mt-7 grid gap-3 sm:grid-cols-3">
                      {['Citation provenance', 'Update awareness', 'Completeness boundary'].map((item) => (
                        <div key={item} className="rounded-xl border border-white/8 bg-black/10 p-3">
                          <span className="font-mono text-[9px] uppercase tracking-wider text-white/30">Unknown</span>
                          <p className="mt-2 text-xs font-semibold text-white/55">{item}</p>
                        </div>
                      ))}
                    </div>
                  </motion.div>
        ...
        <p className="mt-4 text-right font-mono text-[9px] uppercase tracking-wider text-ink-faint">
