    ...
    <div className="relative mx-auto w-full max-w-[1240px]" aria-label="Live Factum Trace showing a clinical question transformed into a verified answer">
      ...
      <div className="glass-panel relative overflow-hidden rounded-[30px] border-white/70 bg-white/80 p-2 shadow-[0_40px_120px_rgba(16,37,66,.16)] sm:rounded-[38px] sm:p-4">
        <div className="flex min-h-12 items-center justify-between gap-3 border-b border-ink/10 px-3 pb-3 pt-1 sm:px-4">
          ...
          <div className="flex items-center gap-2 rounded-full bg-success/10 px-3 py-1.5 font-mono text-[10px] font-bold uppercase tracking-wider text-success sm:text-[11px]">
        ...
        <div className="p-2 sm:p-4 lg:p-5">
          <div className="rounded-[22px] border border-ink/10 bg-ink px-4 py-4 text-white shadow-xl sm:flex sm:items-center sm:gap-5 sm:px-6 sm:py-5">
            <div className="flex shrink-0 items-center gap-2 font-mono text-[10px] uppercase tracking-[0.12em] text-cyan sm:w-[136px] sm:text-[11px]">
            ...
            <button
              ...
              className="mt-4 inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-full bg-factum px-5 font-mono text-[11px] font-bold uppercase tracking-[0.08em] text-ink transition hover:-translate-y-0.5 hover:bg-[#ffd978] disabled:cursor-wait disabled:opacity-65 sm:mt-0"
            >
            ...
            {steps.map((step, index) => {
              ...
              return (
                <div key={step.number} className={`relative z-10 flex items-center gap-2 rounded-xl px-2 py-2 transition-colors sm:justify-center ${active ? 'bg-white shadow-sm' : ''}`}>
                  <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full border font-mono text-[10px] font-bold transition-all duration-500 ${complete ? 'border-cyan-deep bg-cyan-deep text-white' : active ? 'border-factum bg-factum text-ink shadow-[0_0_0_5px_rgba(246,200,95,.18)]' : 'border-ink/15 bg-white text-ink-faint'}`}>{complete ? '✓' : step.number}</span>
                  <span className={`font-mono text-[10px] font-bold uppercase tracking-[0.06em] ${active || complete ? 'text-ink' : 'text-ink-faint'}`}>{step.label}</span>
                </div>
              )
            })}
          </div>
          ...
          <div className="trace-stage soft-grid relative mt-4 grid overflow-hidden rounded-[24px] border border-ink/10 bg-white/60 lg:grid-cols-[.8fr_1fr_1.3fr]">
            <section className="border-b border-ink/10 p-4 sm:p-5 lg:border-b-0 lg:border-r">
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-[10px] uppercase text-ink-faint">2 live inputs</span>
              </div>
              ...
              <div className={`mt-4 rounded-2xl border border-dashed p-3 transition-all duration-500 ${phase >= 1 ? 'border-violet/35 bg-violet/5 opacity-100' : 'border-ink/10 opacity-45'}`}>
                <p className="font-mono text-[10px] font-bold uppercase tracking-[0.08em] text-violet">Resolved intent</p>
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {['AML', '1st line', 'older adult', 'non-intensive'].map((item) => <span key={item} className="rounded-full border border-violet/15 bg-white px-2.5 py-1.5 font-mono text-[10px] text-ink-muted">{item}</span>)}
                </div>
              </div>
            </section>
            ...
            <section className="relative border-b border-ink/10 p-4 sm:p-5 lg:border-b-0 lg:border-r">
              <div className="flex items-center justify-between gap-3">
                ...
                <p aria-live="polite" className="text-right font-mono text-[10px] uppercase tracking-[0.06em] text-cyan-deep">{currentStep}</p>
              </div>
                ...
                {processors.map((processor, index) => {
                  ...
                  return (
                    <motion.div key={processor.name} animate={{ opacity: phase >= processorPhase ? 1 : 0.48, x: active ? 5 : 0 }} transition={{ duration: 0.4 }} className={`relative flex min-h-[94px] items-center gap-3.5 rounded-2xl border bg-white p-4 transition-shadow ${active ? 'border-factum/70 shadow-[0_12px_28px_rgba(16,37,66,.10)]' : complete ? 'border-cyan-deep/20' : 'border-ink/8'}`}>
                      ...
                      <div><p className="text-[15px] font-bold text-ink sm:text-base">{processor.name}</p><p className="mt-1 text-xs leading-5 text-ink-muted sm:text-[13px]">{processor.detail}</p></div>
            ...
            <section className="relative flex min-h-[410px] flex-col p-4 sm:p-5 lg:min-h-0">
              <div className="flex items-center justify-between">
                ...
                <span className={`rounded-full px-2.5 py-1.5 font-mono text-[10px] font-bold uppercase tracking-wider transition-colors ${phase >= 4 ? 'bg-success/10 text-success' : 'bg-ink/5 text-ink-faint'}`}>{phase >= 4 ? 'Verified' : 'Assembling'}</span>
              </div>
                ...
                <AnimatePresence mode="wait">
                  {phase < 4 ? (
                    <motion.div key="waiting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -8 }} className="flex w-full flex-col justify-center rounded-[22px] border border-dashed border-ink/15 bg-paper/70 p-5">
                      ...
                      <p className="mx-auto mt-2 max-w-[270px] text-center text-[13px] leading-5 text-ink-muted">Claims appear only after structure, recency, and provenance checks complete.</p>
                    </motion.div>
                  ) : (
                    <motion.div key="factum" initial={{ opacity: 0, scale: 0.96, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ type: 'spring', stiffness: 180, damping: 20 }} className="factum-shimmer w-full overflow-hidden rounded-[22px] border border-factum/55 bg-ink p-5 text-white shadow-[0_22px_55px_rgba(16,37,66,.25)]">
                      <div className="flex items-center justify-between gap-3"><span className="eyebrow text-factum">Factum 04</span><span className="rounded-full bg-success/20 px-2.5 py-1.5 font-mono text-[10px] font-bold text-success-light">VERIFIED</span></div>
                      <p className="mt-4 text-base font-semibold leading-7 sm:text-lg">For older adults ineligible for intensive induction, the guideline-backed regimen set remains intact.</p>
                      <p className="mt-3 text-[13px] leading-6 text-white/70 sm:text-sm">A newer FDA approval is isolated as a late-breaking update for clinician review, not silently merged into guideline consensus.</p>
                      <div className="mt-4 rounded-xl border border-factum/20 bg-factum/10 p-3.5"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.08em] text-factum">Uncertainty boundary preserved</p><p className="mt-1.5 text-xs leading-5 text-white/65">No claim extends beyond retrieved evidence.</p></div>
...
function SourceCard({ active, tag, title, meta, accent, note }: { active: boolean; tag: string; title: string; meta: string; accent: 'cyan' | 'factum'; note: string }) {
  ...
  return (
    <motion.div animate={{ opacity: active ? 1 : 0.45, y: active ? 0 : 3 }} transition={{ duration: 0.45 }} className={`rounded-2xl border bg-white p-4 transition-shadow ${active ? 'border-ink/12 shadow-[0_10px_25px_rgba(16,37,66,.08)]' : 'border-ink/8'}`}>
      <div className="flex items-start gap-3"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl font-mono text-[10px] font-bold ${accentClasses}`}>{tag.slice(0, 3)}</span><div className="min-w-0"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.1em] text-ink-faint">{tag}</p><p className="mt-1 text-[13px] font-bold leading-5 text-ink sm:text-sm">{title}</p><p className="mt-1 font-mono text-[11px] leading-4 text-ink-faint">{meta}</p></div></div>
      <div className={`mt-3 flex items-center gap-2 border-t border-ink/8 pt-2.5 font-mono text-[11px] leading-4 ${active ? 'text-success' : 'text-ink-faint'}`}><span className={`h-1.5 w-1.5 rounded-full ${active ? 'bg-success' : 'bg-ink/20'}`} /> {active ? note : 'Waiting for trace'}</div>
    </motion.div>
  )
}

function CitationDot({ color, label }: { color: string; label: string }) {
  return <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-2 font-mono text-[10px] text-white/70"><span className={`h-1.5 w-1.5 rounded-full ${color}`} /> {label}</span>
}

function TraceMetric({ value, label }: { value: string; label: string }) {
  return <div className="px-2"><p className="font-mono text-base font-bold text-ink">{value}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-[0.06em] text-ink-faint">{label}</p></div>
}
